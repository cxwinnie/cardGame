...
class JavaExample
{
    ...
    public void optimizeExample(Context ctx)
    {
        ...
        // Set constraints.
        IntExpr xExp = ctx.mkIntConst("x");  //设置x为int常亮
        IntExpr yExp = ctx.mkIntConst("y");

        opt.Add(ctx.mkEq(ctx.mkAdd(xExp, yExp), ctx.mkInt(10)),  //x+y==10
                ctx.mkGe(xExp, ctx.mkInt(0)),   //x>=0
                ctx.mkGe(yExp, ctx.mkInt(0)));  //y>=0

        // Set objectives.
        Optimize.Handle mx = opt.MkMaximize(xExp);  //取x的最大值
        Optimize.Handle my = opt.MkMaximize(yExp);  //接着再取y最大值
...
//                p.floatingPointExample1(ctx);
            ...
            { // These examples need proof generation turned on.
                ...
                cfg.put("proof", "true");
                Context ctx = new Context(cfg);
//                p.proveExample1(ctx);   //implies
//                p.proveExample2(ctx);
//                p.arrayExample2(ctx);
//                p.tupleExample(ctx);
//                p.enumExample(ctx);    //or
//                p.listExample(ctx);
//                p.treeExample(ctx);
//                p.forestExample(ctx);
